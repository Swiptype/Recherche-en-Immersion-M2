    ...
    def __init__(self, texts, tokenizer, max_len):
        ...
        for t in texts:
            if isinstance(t, str) and len(t) > 50:
                # on eleve les stop words pour eviter qu'ils soient considerés comme des rationales
                words = [w for w in t.split() if w.lower() not in ENGLISH_STOP_WORDS]
...
class IGExplainer:
    ...
    def explain_and_visualize(self, text, threshold):
            ...
            # --- MODIFICATION: Liste de mots à ignorer visuellement ---
            STOP_WORDS = set(['the', 'a', 'an', 'and', 'or', 'if', 'is', 'are', 'was', 'were', 
            ...
            for token, weight in zip(tokens, attrs_norm):
                ...
                check_t = clean_t.lower() # On vérifie le mot en minuscule
                
                # SI c'est un stop word, on force la couleur DIM (gris), sinon on applique la logique de poids
                if check_t in STOP_WORDS:
                    color = Style.DIM
                elif weight > 0.7: 
